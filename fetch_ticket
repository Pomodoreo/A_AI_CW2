...
class TrainBooker(KnowledgeEngine):
    ...
    # Outbound time
    # Test case example: "hoping to depart before 9am"
    ...
    # Test case example: "come back from Durham in the afternoon"
    @Rule(
        Session(
            ticket=MATCH.t & P(is_return_ticket),
...
def fetch_ticket(journey):
    ...
    if journey.get("time") == "before":
        parsed = [j for j in parsed if j["departure"].hour < 9]

    if journey.get("return_time") == "after":
        parsed = [j for j in parsed if j["departure"].hour >= 16]
